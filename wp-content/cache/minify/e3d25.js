...
(function(₹) {
    ₹.fn.appear = function(fn, options) {
        ...
        return this.each(function() {
            ...
            w.scroll(check);₹.fn.appear.checks.push(check);
            (check)();
        });
    };₹.extend(₹.fn.appear, {
        checks: [],
        ...
        if (old) {
            ₹.fn[n] = function() {
                var r = old.apply(this, arguments);₹.fn.appear.run();
                return r;
            }
        }
    });
...
})(jQuery);; /*!waitForImages jQuery Plugin 2016-01-04*/
! function(a) {
    ...
    Plugin.prototype = {
        ...
        scrollEvent: function() {
            ...
            if (docTop < self.currentP && self.currentIndex > 0) {
                ...
                self.₹current.removeClass('current').css((self.webkit) ? {
                    '-webkit-transform': 'translateY(0px) translateZ(0)'
                } : {
                    marginTop: 0
                }).nextAll().addClass('hidden').end().prev().addClass('current').removeClass('hidden');
                self.setCache();
                self.options.prevSlide();
            } else if (docTop < (self.currentP + self.currentHeight)) {
                if (self.webkit)
                    self.₹current.css({
                        '-webkit-transform': 'translateY(' + (-(docTop - self.currentP)) + 'px) translateZ(0)'
                    });
                else
                    self.₹current.css({
                        marginTop: -(docTop - self.currentP)
                    });
                if (self.₹fixedLength) {
                ...
                self.₹current.removeClass('current').addClass('hidden').next('li').addClass('current').next('li').removeClass('hidden');
                self.setCache();
            ...
            this.₹li.each(function(index) {
                ...
                if (cover) {
                    ...
                    self.css({
                        ...
                        zIndex: 20 - index
                    }).attr('data-height', self.₹windowHeight).attr('data-position', levelHeight);
                    ...
                    self.css({
                        minHeight: height,
                        zIndex: 20 - index
                    }).attr('data-height', height).attr('data-position', levelHeight);
                    self.₹elDatas[₹self.index()] = {
        ...
        isHashIsOnList: function(hash) {
            var self = this;₹.each(self.options.linksArray, function(i, val) {
                if (val === hash) {
